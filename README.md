...
| `hooks/` | ホームディレクトリ配下で参照する Claude hooks 一式 |
| `commands/` | ホームディレクトリ配下で使う Claude commands 一式 |
...
### `hooks/commands` の配備方針

この Role は、ホームディレクトリ配下に Claude 用の補助ファイルも配備します。

- `hooks/` は [toshibe/.claude/hooks](toshibe/.claude/hooks) をもとに配備
- `commands/` は [toshibe/.claude/commands](toshibe/.claude/commands) をもとに配備
- Ubuntu / Windows のどちらでも、`settings.json` からは `$HOME/.claude/hooks/*.sh` を `bash` 経由で呼び出す前提
